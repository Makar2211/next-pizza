					...
					>
						{
							products.map(product => (
								<Link key={product.id} onClick={onClickItem} href={`/product/${product.id}`} className="flex items-center gap-2 w-full px-3 py-2 hover:bg-primary/10">
									<img className="rounded-sm h-8 w-8" src={product.imageUrl} alt={product.name} />
									<div className="flex items-center gap-3 w-full px-3 py-2">{product.name}</div>
								</Link>
							))
						}
